const moviesData = {
 ...
 const filmsData = {
  "films": [
    { "id": "1", "title": "The Giant Gila Monster" },
    { "id": "2", "title": "Manos: The Hands Of Fate" },
    { "id": "3", "title": "Time Chasers" },
    { "id": "4", "title": "The Touch Of Satan" },
    { "id": "5", "title": "Santa Claus Conquers The Martians" },
    { "id": "6", "title": "Track Of The Moon Beast" },
    { "id": "7", "title": "The Skydivers" },
    { "id": "8", "title": "The Killer Shrews" },
    { "id": "9", "title": "Project Moon Base" },
    { "id": "10", "title": "The Giant Spider Invasion" },
    { "id": "11", "title": "Catalina Caper" },
    { "id": "12", "title": "Secret Agent Super Dragon" },
    { "id": "13", "title": "Wild Rebels" },
    { "id": "14", "title": "Danger: Diabolik" },
    { "id": "15", "title": "Village Of The Giants" }
  ]
};

const ulElement = document.getElementById('films');

// Remove placeholder <li> if it exists
const placeholder = ulElement.querySelector('li');
if (placeholder) placeholder.remove();

filmsList.addEventListener('click', (event) => {
  const selectedLi = event.target.closest('li[data-id]');
  if (selectedLi) {
      const selectedMovieId = selectedLi.getAttribute('data-id');
      const movie = moviesData.films.find(m => m.id === selectedMovieId);
      if (movie) updateMovieDetails(movie);
  }
});


function populateMoviesList(movies, targetElementId) {
  const ulElement = document.getElementById(targetElementId);
...
populateMoviesList(moviesData.films, 'films');


// Handle movie selection and display details
filmsList.addEventListener('click', (event) => {
  const selectedMovieId = event.target.getAttribute('data-id');
  const movie = moviesData.films.find(movie => movie.id === selectedMovieId);

  if (movie) {
    document.getElementById('title').textContent = movie.title;
    document.getElementById('runtime').textContent = `${movie.runtime} minutes`;
    document.getElementById('showtime').textContent = movie.showtime;
    document.getElementById('tickets').textContent = `${movie.capacity - movie.tickets_sold} tickets available`;
    document.getElementById('poster').setAttribute('src', movie.poster);
    document.getElementById('poster').setAttribute('alt', `${movie.title} Poster`);
    document.getElementById('buy-ticket').disabled = movie.tickets_sold >= movie.capacity;
  }
});
